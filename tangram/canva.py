...
class TangramCanvas:
    ...
    def create_polygon(self, _type, label, coord):
        ...
        if coord == 0:
            ...
            if (
                CanvasUtils.label_value(label) + 1 <= _max
                and CanvasUtils.label_value(label) >= 0
            ):
                # Creating a new Polygon
                CanvasPolygon(
                    coords, color, _type, self.drawing_place, self.magnet_slider
                )
                CanvasUtils.update_count_label(label, "+")
            else:
                messagebox.showerror(
                    "Maximum reached", "You've reached the maximum amount of this item."
                )
        else:
    ...
    def delete_polygon(self, _type, label):
        ...
        if CanvasUtils.label_value(label) > 0:
            ...
            messagebox.showerror(
                "Minimum reached", "You've reached the minimum amount of this item."
            )
    ...
    def transform_coords(self):
        ...
        if len(figures) >= 1:
            for fig in figures:
                ...
                coords.append(
                    utils.divide_coords(
                        utils.tuple_to_list(self.drawing_place.coords(fig)), 100
                    )
                )
            ...
            for sub_ref in multipolygon:
                ...
                CanvasPolygon(
                    xy, "black", "origin", self.drawing_place, self.magnet_slider, False
                )
                print(xy)

            print(types)

            should_reset = SolvingThread.solve(
                output, types, self.window, self.progress
            )
            if should_reset:
                CanvasUtils.reset_canvas(self.drawing_place, self.labels)
    ...
    def models(self, index):
        ...
        for i, coord in enumerate(coords):
            self.polygon_action(
                _type[i], self.get_label_by_type(_type[i]), "add", coord
            )
    ...
    def setup_prefabs(self):
        ...
        for i, elem in enumerate(self.prefabs):
            ...
            self.prefabs_models.append(
                PhotoImage(file="../images/" + elem["name"] + ".PNG")
            )
            self.prefabs_buttons.append(
                Button(
                    self.scframe.interior,
                    image=self.prefabs_models[i],
                    relief=FLAT,
    ...
    def __init__(self):
        ...
        self.numPolygonsFrame = LabelFrame(
            self.window, text="Number of Polygons", padx=14, pady=10, labelanchor="nw"
        )
        self.numPolygonsFrame.place(x=71, y=50)

        self.magnetFrame = LabelFrame(
            self.window,
            text="Magnetization distance",
            padx=10,
            pady=5,
            labelanchor="nw",
        )
        ...
        self.commandFrame = LabelFrame(
            self.window, text="Actions", padx=5, pady=5, labelanchor="nw"
        )
        self.commandFrame.place(x=80, y=350)

        self.polygonsLimitsFrame = LabelFrame(
            self.window, text="Polygon's limits", padx=5, pady=5, labelanchor="nw"
        )
        ...
        # + or - buttons
        self.plusButton4 = Button(
            self.numPolygonsFrame,
            text="+",
            fg="green",
            width=3,
            justify=CENTER,
            command=lambda: [
                self.polygon_action("bt", self.bigTriangleLabel, "add", 0)
            ],
        )
        ...
        self.minusButton4 = Button(
            ...
            justify=CENTER,
            command=lambda: [
                self.polygon_action("bt", self.bigTriangleLabel, "del", 0)
            ],
        )

        self.plusButton5 = Button(
            self.numPolygonsFrame,
            text="+",
            fg="green",
            width=3,
            justify=CENTER,
            command=lambda: [
                self.polygon_action("p", self.parallelogramLabel, "add", 0)
            ],
        )
        ...
        self.minusButton5 = Button(
            ...
            justify=CENTER,
            command=lambda: [
                self.polygon_action("p", self.parallelogramLabel, "del", 0)
            ],
        )

        self.plusButton1 = Button(
            self.numPolygonsFrame,
            text="+",
            fg="green",
            width=3,
            justify=CENTER,
            command=lambda: [
                self.polygon_action("mt", self.medTriangleLabel, "add", 0)
            ],
        )
        ...
        self.minusButton1 = Button(
            ...
            justify=CENTER,
            command=lambda: [
                self.polygon_action("mt", self.medTriangleLabel, "del", 0)
            ],
        )

        self.plusButton3 = Button(
            self.numPolygonsFrame,
            text="+",
            fg="green",
            width=3,
        ...
        self.plusButton2 = Button(
            ...
            justify=CENTER,
            command=lambda: [
                self.polygon_action("st", self.smallTriangleLabel, "add", 0)
            ],
        )
        ...
        self.minusButton2 = Button(
            ...
            justify=CENTER,
            command=lambda: [
                self.polygon_action("st", self.smallTriangleLabel, "del", 0)
            ],
        )
        ...
        self.btn_validate = Button(
            self.commandFrame,
            text="Validate",
            command=lambda: [self.transform_coords()],
        )
