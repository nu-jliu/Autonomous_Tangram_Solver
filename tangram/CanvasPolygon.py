...
class CanvasPolygon:
    ...
    def __init__(self, coords, color, tag, canvas, magnet_slider, movable=True):
        ...
        self.id_ = canvas.create_polygon(
            list(self.coords), fill=self.color, tags=self.tag
        )
    ...
    def movement(self, event):
        ...
        if self.move:
            end_x = self.canvas.canvasx(
                event.x
            )  # Translate mouse x screen coordinate to canvas coordinate
            end_y = self.canvas.canvasy(
                event.y
            )  # Translate mouse y screen coordinate to canvas coordinate
    ...
    def stop_movement(self, event):
        ...
        if nearby:
            ...
            CanvasPolygon(
                CanvasUtils.replace(
                    utils.tuple_to_list(self.coords), utils.tuple_to_list(nearby)
                ),
                self.color,
                self.tag,
                self.canvas,
    ...
    def rotate(self, event):
        ...
        for x_pos, y_pos in zip(alpha, beta):
            ...
            new_points.append((tx_pos * cos_ang + ty_pos * sin_ang) + points[0])
            new_points.append((-tx_pos * sin_ang + ty_pos * cos_ang) + points[1])
