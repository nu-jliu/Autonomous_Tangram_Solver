...
def rotate_by(x_pivot, y_pivot, x_pos, y_pos, angle):
    ...
    return [
        math.cos(angle) * (x_pos - x_pivot)
        - math.sin(angle) * (y_pos - y_pivot)
        + x_pivot,
        math.sin(angle) * (x_pos - x_pivot)
        + math.cos(angle) * (y_pos - y_pivot)
        + y_pivot,
    ]
...
def fit_function(types, state, ref):
    ...
    for i, shape in enumerate(state):
        polygons = unary_union(
            [
                polygons,
                get_shape_polygon_by_index(
                    types, i, shape[0], shape[1], shape[2], shape[3]
                ),
            ]
        )
...
def get_shape_polygon_by_index(
    shapes, index, x_pos, y_pos, rotation, point_index, offset=0.005
):
    ...
    if shapes[index] == "bt":
        ...
        return center_index(
            Polygon(
                get_parallelogram_points(x_pos, y_pos, rotation, side / 2 - offset)
            ),
            point_index,
        )
