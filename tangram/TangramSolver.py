            ...
            for i in range(len(sub_ref.exterior.xy[0])):
                ...
                for r in range(PolygonUtils.get_rot_by_index(self.shapes, shape_index)):
                    ...
                    for point_index in range(
                        PolygonUtils.get_corner_count_by_index(self.shapes, shape_index)
                    ):

                        new_state = copy.deepcopy(state)
                        new_state.append([x, y, r, point_index])
                        poly = PolygonUtils.get_shape_polygon_by_index(
                            self.shapes, shape_index, x, y, r, point_index
                        )
                        ...
                        if utils.margin_error(diff.area, ref.area - poly.area) < 0.35:
                            ...
                            if shape_index == len(self.shapes) - 1:
                                ...
                                print("continue")
                                is_success, new_state = self.corner_explore(
                                    diff, shape_index + 1, new_state
                                )
                                if is_success:
                                    return True, new_state
