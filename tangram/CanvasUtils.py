    ...
    if original and change:
        ...
        for i, p in enumerate(original):
            if p == change[0][0]:
                if original[
                    i + 1
                ]:  # prevents a list index out of range error from happening
...
def get_settings_by_type(_type, x_pos, y_pos):
    ...
    if _type == "bt":
        return (
            MAX_BIG_TRIANGLE,
            numpy.array(PolygonUtils.get_triangle_points(x_pos, y_pos, 0, unit * 2))
            .flatten()
            .tolist(),
        )
    elif _type == "p":
        return (
            MAX_PARALLELOGRAM,
            numpy.array(
                PolygonUtils.get_parallelogram_points(x_pos, y_pos, 0, side / 2)
            )
            .flatten()
            .tolist(),
        )
        ...
        return (
            MAX_MED_TRIANGLE,
            numpy.array(PolygonUtils.get_triangle_points(x_pos, y_pos, 0, side))
            .flatten()
            .tolist(),
        )
    elif _type == "s":
        return (
            MAX_SQUARE,
            numpy.array(PolygonUtils.get_square_points(x_pos, y_pos, 0, unit))
            .flatten()
            .tolist(),
        )
    elif _type == "st":
        return (
            MAX_SM_TRIANGLE,
            numpy.array(PolygonUtils.get_triangle_points(x_pos, y_pos, 0, unit))
            .flatten()
            .tolist(),
        )
